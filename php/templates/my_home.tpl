	...
	<div class="panel-body">
		...
		<p>
			<center>
				...
				<div style="width: 340px;padding-bottom: 7px;">
				...
				{if $current}
				<div style="width: 340px;padding-bottom: 7px;">
					<a href="?action=event&function=event_view&event_id={$current.event_id|escape}" class="btn btn-block btn-warning btn-rounded" style="font-size: 20px;">
						<i class="fa fa-thumbs-o-up" style="float:left;padding-top: 3px;"></i>
						I Am At This Event Now!
					</a>
				</div>
				{/if}
				...
				<div style="width: 340px;padding-bottom: 7px;">
						...
						<ul class="dropdown-menu dropdown-menu-left" style="width:100%;position: relative;font-size:16px;">
							{foreach $future as $f}
								<li><a href="?action=event&function=event_view&event_id={$f.event_id|escape}">{$f.event_start_date|date_format:"%y-%m-%d"} - {$f.event_name|truncate:"30"|escape}</a></li>
							{/foreach}
						</ul>
				</div>
				{/if}
				...
				<div style="width: 340px;padding-bottom: 7px;">
						...
						<ul class="dropdown-menu dropdown-menu-left" style="width:100%;position: relative;font-size:16px;">
							{foreach $past as $p}
								<li><a href="?action=event&function=event_view&event_id={$p.event_id|escape}">{$p.event_start_date|date_format:"%y-%m-%d"} - {$p.event_name|truncate:"30"|escape}</a></li>
							{/foreach}
						</ul>
				</div>
				{/if}
				...
				<div style="width: 340px;padding-bottom: 7px;">
						<button class="btn btn-block btn-success btn-rounded dropdown-toggle" style="font-size: 20px;" data-toggle="dropdown" aria-expanded="false">
							<i class="fa fa-trophy" style="float:left;padding-top: 3px;"></i>
							My Series
							<i class="dropdown-caret fa fa-chevron-down"style="padding-top: 3px;padding-left: 15px;"></i>
						</button>
						<ul class="dropdown-menu dropdown-menu-left" style="width:100%;position: relative;font-size:16px;">
							{foreach $series as $s}
								<li><a href="?action=series&function=series_view&series_id={$s.series_id|escape}">{$s.series_name|truncate:"36"|escape}</a></li>
							{/foreach}
						</ul>
				</div>
				{/if}
