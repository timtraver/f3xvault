<div class="page type-page status-publish hentry clearfix post nodate">
	<div class="entry clearfix">                

<h1 class="post-title entry-title">F3X Vault User Password Change</h1>
<br \>
<center>
<form name="login" method="POST" autocomplete="off">
<input type="hidden" name="action" value="my">
<input type="hidden" name="function" value="change_password">
<table width="40%" cellpadding="2" cellspacing="1">
<tr>
	<th colspan="2">
		F3X Vault Password Change
	</th>
</tr>
<tr>
	<td align="left">
	<center>        
	<table cellpadding="3" cellspacing="1">        
	<tr>
		<th valign="top">Existing Password</th>
		<td>
			<input type="password" name="pass" size="30" class="text" autocomplete="off"><br>
		</td>
	</tr>
	<tr>
		<th valign="top">New Password</th>
		<td>
			<input type="password" name="pass1" size="30" class="text" autocomplete="off"><br>
		</td>
	</tr>
	<tr>
		<th valign="top">Verify New Password</th>
		<td>
			<input type="password" name="pass2" size="30" class="text" autocomplete="off"><br>
		</td>
	</tr>
	</table>
		<div style="padding-top:5px;"><input type="submit" value=" Change My Password " class="button"></div>
	</center>
	</td>
</tr>
</table>
</form>
</center>


	</div>
</div>