...
{include file='print/print_draw_header.tpl'}
		...
		<h2 class="post-title entry-title" style="margin:0px;">Draw Matrix - {if $event->info.event_type_code!="f3k"}{$event->flight_types.$flight_type_id.flight_type_name} {/if}(Rounds {$print_round_from} - {$print_round_to})</h2>
			...
			Team Highlight :
			<select name="highlight" onChange="document.main.submit();">
			...
			&nbsp;&nbsp;&nbsp;&nbsp;Include Country 
			<input type="checkbox" name="show_country"{if $show_country == 1} CHECKED{/if} onChange="document.main.submit();">
			</form>
		<br>
		...
		<tr>
			...
			{if $event->flight_types.$ftid.flight_type_code == 'f3b_duration' ||
				$event->flight_types.$ftid.flight_type_code == 'td_duration' ||
				$event->flight_types.$ftid.flight_type_code == 'f3b_distance' ||
				$event->flight_types.$ftid.flight_type_code == 'f3j_duration' ||
				$event->flight_types.$ftid.flight_type_code == 'f3l_duration' ||
				$event->flight_types.$ftid.flight_type_code == 'f5j_duration'}
				{$size=3}
			{else}
				{$size=2}
			{/if}
			...
			<td>
				...
				<tr bgcolor="lightgray">
					{if $event->flight_types.$ftid.flight_type_group && $event->flight_types.$ftid.flight_type_code!='f3f_speed'}
					...
					{if $event->flight_types.$ftid.flight_type_code=='f3b_duration' 
						...
						<td>Lane</td>
					{/if}
					{if $event->flight_types.$ftid.flight_type_code=='f3l_duration' }
						<td>Team</td>
					{/if}
				</tr>
				{$oldgroup='1000'}
				{$bottom=0}
				{foreach $r.flights.$ftid.pilots as $p}
					{$event_pilot_id=$p@key}
					{if $oldgroup!=$p.event_pilot_round_flight_group}
						{if $r.flights.$ftid.flight_type_code=='f3b_speed' || $r.flights.$ftid.flight_type_code=='f3f_speed'}
							{$bgcolor="white"}
						{else}
							{if $bgcolor=='white'}
								{$bgcolor='lightgray'}
						...
						{$bottom=1}
					{/if}
					{if $event->pilots.$event_pilot_id.event_pilot_team==$highlight && $event->pilots.$event_pilot_id.event_pilot_team!=''}
						{$highlighted=1}
					{else}
						{$highlighted=0}
					{/if}
