	...
	<div class="panel">
		<div class="panel-heading">
			...
			<div style="float:right;overflow:hidden;margin-top:10px;">
				<input type="button" value=" Reload " onClick="document.reload.submit();"
					class="btn btn-primary btn-rounded" style"float:right;">
				<input type="button" value=" Edit Series Parameters " onClick="document.edit_series.submit();"
					class="btn btn-primary btn-rounded" style"float:right;">
				<input type="button" value=" Back To Series List " onClick="document.goback.submit();"
					class="btn btn-primary btn-rounded" style"float:right;">
			</div>
		</div>
			...
			<form name="series_save_multiples" method="POST">
				...
				<table width="100%" cellpadding="0" cellspacing="0" class="table table-condensed table-bordered">
					...
					<tr>
						...
						<td>
							{$series->info.series_area|escape},{if $series->info.state_code != NULL}{$series->info.state_code|escape}{/if}
							{$series->info.country_code|escape}
							{if $series->info.country_code}<img
									src="/images/flags/countries-iso/shiny/24/{$series->info.country_code|escape}.png"
								style="vertical-align: middle;">{/if}
							{if $series->info.state_name != 'Other' && $series->info.country_code=="US"}<img
									src="/images/flags/states/24/{$series->info.state_name|replace:' ':'-'}-Flag-24.png"
								style="vertical-align: middle;">{/if}
						</td>
					</tr>
					<tr>
						<th style="text-align:right;">Series Web URL</th>
						<td><a href="{$series->info.series_url}" target="_new"
								class="btn-link">{$series->info.series_url}</a></td>
					</tr>
					<tr>
					...
					<table width="100%" cellpadding="1" cellspacing="1" class="table-striped table-series">
						...
						{foreach $series->events as $e}
							<tr>
								...
								<td>
									<a href="?action=event&function=event_view&event_id={$e.event_id|escape:'url'}"
										class="btn-link">{$e.event_name|escape}</a>
								</td>
								<td>{$e.location_name|escape}</td>
								<td nowrap>
									{if $e.state_name && $e.country_code=="US"}<img
											src="/images/flags/states/16/{$e.state_name|replace:' ':'-'}-Flag-16.png"
										style="vertical-align: middle;">{/if}
									{$e.state_name|escape}
								</td>
								<td nowrap>
									{if $e.country_code}<img
											src="/images/flags/countries-iso/shiny/16/{$e.country_code|escape}.png"
										style="vertical-align: middle;">{/if}
									{$e.country_name|escape}
								</td>
								<td nowrap>{$e.total_pilots|escape}</td>
								<td nowrap>
									<input type="text" name="multiple_{$e.event_series_id|escape}" size="6"
										value="{$e.event_series_multiple|escape}">
									<input type="checkbox" name="mandatory_{$e.event_series_id|escape}"
										{if $e.event_series_mandatory}CHECKED{/if}> Mandatory
								</td>
							</tr>
							{$num=$num+1}
						{/foreach}
				...
				<br>
				<input type="button" value=" Save Series Parameters " onClick="document.series_save_multiples.submit();"
					class="btn btn-primary btn-rounded" style="float:right;">
			</form>
					...
					<thead>
						<tr>
							...
							<th colspan="{$series->totals.total_events + 1}" align="center" nowrap>
								...
								{/foreach}
								{if $best > 0}
									Series Events ( Best {$best|escape} out of {$series->completed_events|escape} Completed
									Events )
								{else}
									Series Events
									({if $series->totals.round_drops==0}No{else}{$series->totals.round_drops|escape}{/if}
									Drop{if $series->totals.round_drops!=1}s{/if} In Effect over
									{$series->completed_events|escape} Completed Events)
								{/if}
							</th>
						</tr>
						<tr>
							...
							{foreach $series->events as $e}
								<th width="1%" align="center" style="text-align: center;" nowrap>
									<a class="tooltip_score_right_low"
						...
						{foreach $series->totals.pilots as $p}
							...
							<tr>
								...
								<td align="right" nowrap>
									<a href="?action=series&function=series_pilot_view&pilot_id={$pilot_id|escape:'url'}&series_id={$series->info.series_id|escape:'url'}"
										class="tooltip_series_score">
										{if $p.excluded==true}<font color="red"><strike>{/if}
												{$p.pilot_first_name|escape} {$p.pilot_last_name|escape}
												{if $p.excluded==true}</strike></font>{/if}
										<span>
											{if $p.excluded==true}
												<b>Pilot Exclluded From Results</b> <br>
											{/if}
										</span>
									</a>
								</td>
								...
								{foreach $series->events as $e}
									...
									<td class="info" align="right" {if $e.pilots.$pilot_id.event_pilot_position==1}
										style="border-width: 2px;border-color: green;color:green ;font-weight:bold;" {/if}>
										<div style="position:relative;">
											<a href="" class="tooltip_series_score" onClick="return false;">
												...
												<span {if $series->info.series_scoring_type!='f5jtour'}style="margin-top: -40px; "
													{/if}>
													{if $series->info.series_scoring_type=='faiwc'}
														{$p.events.$event_id.event_score|string_format:"%d"}
													{elseif $series->info.series_scoring_type=='position' || $series->info.series_scoring_type=='teamusa'}
													...
													{else if $series->info.series_scoring_type=='f5jtour'}
														<table>
															<tr>
																<th>Prelim</th>
																<td align="right">
																	{$p.events.$event_id.event_score_orig|string_format:"%0.2f"}</td>
															</tr>
															<tr>
																<th>Bonus</th>
																<td align="right">
																	{$p.events.$event_id.event_score_bonus|string_format:"%0.2f"}</td>
															</tr>
														</table>
													{else}
														<table>
